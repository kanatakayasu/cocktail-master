...
function collectAllPaths(node: BeverageNode, prefix = ''): string[] {
...
function getLevelColor(level: number, node: BeverageNode): string {
...
export default function ClassificationPage() {
  ...
  const [expanded, setExpanded] = useState<Set<string>>(() => {
    // 初期状態: ルートのみ展開
    return new Set([beverageTree.name]);
  });

  const allPaths = useMemo(() => collectAllPaths(beverageTree), []);
  ...
  const collapseAll = useCallback(() => {
    // ルートのみ残す
    setExpanded(new Set([beverageTree.name]));
  }, []);
  ...
  return (
    ...
    >
      ...
      <div className="text-center mb-8">
        ...
        <h1 className="text-3xl sm:text-4xl font-bold gradient-text mb-3">飲料分類</h1>
        <p className="text-gray-400 text-sm sm:text-base max-w-2xl mx-auto leading-relaxed">
          飲料は大きく<span className="text-accent-red font-semibold">【アルコール飲料】</span>と
          <span className="text-accent-gold font-semibold">【非アルコール飲料】</span>
          に分類されます。アルコール飲料はさらに醸造酒・蒸留酒・混成酒に分かれ、
          それぞれ異なる製法と特徴を持っています。
        </p>
      </div>
      ...
      >
        ...
        <span className="flex items-center gap-1.5">
          <span className="w-2 h-2 rounded-full" style={{ backgroundColor: '#9C27B0' }} />
          醸造酒
        </span>
        <span className="flex items-center gap-1.5">
          <span className="w-2 h-2 rounded-full" style={{ backgroundColor: '#FF5722' }} />
          蒸留酒
        </span>
        <span className="flex items-center gap-1.5">
          <span className="w-2 h-2 rounded-full" style={{ backgroundColor: '#3F51B5' }} />
          混成酒
        </span>
      </motion.div>
    </motion.div>
