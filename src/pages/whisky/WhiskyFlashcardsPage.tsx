...
import { whiskyFlashCards } from '../../data/whisky';
...
const CATEGORIES: CategoryMeta[] = [
  { key: 'w_terms', emoji: '📝', label: '用語', colorBar: 'bg-accent-gold', accent: 'text-accent-gold' },
  { key: 'w_distilleries', emoji: '🏭', label: '蒸留所', colorBar: 'bg-amber-800', accent: 'text-accent-gold' },
  { key: 'w_production', emoji: '⚗️', label: '製造工程', colorBar: 'bg-bar-green-light', accent: 'text-accent-cream' },
  { key: 'w_history', emoji: '📜', label: '歴史', colorBar: 'bg-accent-red', accent: 'text-accent-red' },
  { key: 'w_brands', emoji: '🏷️', label: '銘柄・ブレンド', colorBar: 'bg-accent-cream', accent: 'text-accent-cream' },
  { key: 'w_law', emoji: '⚖️', label: '法規・定義', colorBar: 'bg-bar-green-light', accent: 'text-bar-green-light' },
...
export default function WhiskyFlashcardsPage() {
  ...
  const countByCategory = useMemo(() => {
    ...
    for (const c of whiskyFlashCards) {
      map[c.category] = (map[c.category] ?? 0) + 1;
    }
    map['all'] = whiskyFlashCards.length;
    return map;
  }, []);
  ...
  const startStudy = useCallback((cat: CategoryKey) => {
    const filtered: FlashCard[] =
      cat === 'all' ? whiskyFlashCards : whiskyFlashCards.filter((c: FlashCard) => c.category === cat);
