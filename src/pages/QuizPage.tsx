...
const CATEGORIES: CategoryMeta[] = [
  { id: 'spirits', emoji: '🥃', label: 'スピリッツ', description: '原料・産地・製法' },
  { id: 'cocktails', emoji: '🍹', label: 'カクテル', description: 'レシピ・材料・技法' },
  { id: 'liqueurs', emoji: '🍸', label: 'リキュール', description: '分類・特徴・銘柄' },
  { id: 'tools', emoji: '🔧', label: 'ツール・グラス', description: '名称と用途' },
  { id: 'techniques', emoji: '🎯', label: '技法', description: '特徴と使い分け' },
  { id: 'history', emoji: '📜', label: '歴史', description: '酒類の歴史' },
  { id: 'terms', emoji: '📝', label: '用語', description: '重要用語' },
  { id: 'wine_beer_sake', emoji: '🍷', label: 'ワイン・ビール・日本酒', description: '醸造酒の知識' },
  { id: 'service', emoji: '🤵', label: 'サービス・接客', description: '接客マナー・衛生管理' },
  { id: 'garnish', emoji: '🍋', label: 'ガーニッシュ', description: '飾り付け・フルーツカット' },
...
const STORAGE_KEY = 'cocktail-quiz-scores';
...
function countByDifficulty(catId: string): Record<Difficulty, number> {
  const pool = catId === 'all' ? quizQuestions : quizQuestions.filter((q) => q.category === catId);
    ...
    return (
      ...
      >
        <div className="text-center mb-8">
          <h1 className="text-3xl sm:text-4xl font-bold gradient-text mb-2">クイズに挑戦</h1>
